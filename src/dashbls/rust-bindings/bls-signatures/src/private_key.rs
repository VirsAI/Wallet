...
impl PrivateKey {
    ...
    #[cfg(feature = "virsai_helpers")]
    pub fn generate_virsai(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "virsai_helpers")]
    pub fn generate_virsai_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_virsai(rng))
            .collect()
    }
