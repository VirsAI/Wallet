...
const char * const BITCOIN_CONF_FILENAME = "virsai.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //virsai-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\VirsAI
    // macOS: ~/Library/Application Support/VirsAI
    // Unix-like: ~/.virsai
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VirsAI";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/VirsAI";
#else
    // Unix-like
    return pathRet / ".virsai";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty virsai.conf if it does not exist
