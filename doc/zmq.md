...
The VirsAI Core daemon can be configured to act as a trusted "border
router", implementing the virsai wire protocol and relay, making
...
The ZeroMQ feature in VirsAI Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building virsaid:
...
For instance:
    ...
    $ virsaid -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/virsaid.tx.raw \
...
These options can also be provided in virsai.conf.
...
From the perspective of virsaid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into virsaid directly. Furthermore, no information is
...
using. VirsAId appends an up-counting sequence number to each
